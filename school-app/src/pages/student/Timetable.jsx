...
export default function StudentTimetable() {
  ...
  const periods = timetable[DAYS[activeDay]] || []
  ...
  return (
    <>
      ...
      <div className="page-header">
        ...
        <h1 className="page-header-title">Weekly Schedule</h1>
            ...
            <div key={p.id} className={`period-card ${active ? 'in-progress' : ''} ${done ? 'done' : ''}`}>
              ...
              <div className="period-body">
                ...
                <div className="period-meta-row">
                  <span className="period-meta-item"><PersonIcon /> {p.teacher}</span>
                  <span className="period-meta-item"><RoomIcon /> {p.room}</span>
                </div>
              </div>
...
function getSubjEmoji(subj = '') {
  const map = { Mathematics:'🔢', Science:'🧪', English:'📖', Marathi:'🅰️', History:'🏛️', Geography:'🌍', 'Free Period':'☕' }
